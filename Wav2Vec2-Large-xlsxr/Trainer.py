...
model = Wav2Vec2ForCTC.from_pretrained(
    "/home/theone/other_models/Wav2Vec/out/Base/run3-checkpoint-107500",#"facebook/wav2vec2-large-xlsr-53",    
...
for name, param in model.named_parameters():
    if name.startswith("wav2vec2.encoder.layers.2") |  name.startswith("wav2vec2.encoder.layers.12") |  name.startswith("wav2vec2.encoder.layers.13") | name.startswith("wav2vec2.encoder.layers.14")  | name.startswith("wav2vec2.encoder.layers.15") | name.startswith("wav2vec2.encoder.layers.16") | name.startswith("wav2vec2.encoder.layers.17") | name.startswith("wav2vec2.encoder.layers.18") | name.startswith("wav2vec2.encoder.layers.19"):
         param.requires_grad = True
#    if name.startswith("lm_head"):
#        torch.nn.init.xavier_uniform(model.lm_head.weight)
...
training_args = TrainingArguments(
  ...
  eval_steps=11000,
  logging_steps=100,
  learning_rate=9e-6, #==> 5e-6 ==> 0.98, ### DIMINUIR LEARNING RATE 0.4
  weight_decay=0.0,
  warmup_steps=200,
  save_total_limit=10,
  push_to_hub=False,
