...
turtles-own [neighbor-left neighbor-right neighbor-left-state neighbor-right-state state similar-nearby
  nearest-neighbor total-nearby other-nearby community os-links ]
...
;    ask turtles [ make-node turtle 0]        ;; first node, unattached
 ; ask turtles [ make-node turtle 1 ]       ;; second node, attached to first node
  ...
  layout-spring turtles links 0.3 (world-width / 9) 1
...
to resize-node
    ...
    ask turtles [ set size 0.8 + ( sqrt count link-neighbors ) / 4              ;; size of node according to number of links with neighbors
             ]                                                                  ;;"set label who" to see agents numbers
end

to make-node [old-node]
  if breed = clients [
    set neighbor-left one-of turtles with [ distance myself <= radius-of-interaction ] with [ breed = clients ] ;;with [ count link-neighbors > count my-links ]  ;; choooses one of neighbors inside radius of interaction
