...
to setup
  ...
  ask turtles [
      ...
      ifelse  CA-rule = 30 or CA-rule = 126 or CA-rule = 214 or CA-rule = 232 or CA-rule = 141 [
  ...
  crt int ( N-turtles * percentage-unhappy / 100  )    ; create unhappy agents
    ...
    set neighbor-left-state 0                     ;; state of neighbors that each agent will interact with
    set neighbor-right-state 0
  ...
  crt N-turtles - int ( percentage-unhappy / 100 * N-turtles ) ;create agents with state bigger or equal to 2
    ...
    ifelse  CA-rule = 30 or CA-rule = 126 or CA-rule = 214 or CA-rule = 232  or CA-rule = 141 [
    set state one-of [ 0 1 ]
    set neighbor-left-state 1                     ;; state of neighbors that each agent will interact with
    set neighbor-right-state 1
...
to cellular_automata                                        ;; algorithm for the cellular automata
  ifelse  CA-rule = 30 or CA-rule = 126 or CA-rule = 214 or CA-rule = 232  or CA-rule = 141 [ py:set "base_" 2]
  [ py:set "base_" 5]
    py:set "rule"  CA-rule
    ...
    py:run "regra=int(rule)"      ;; thesis rule  5 states" ;; interesting rule 2159062512564987644819455219116893945895958528152021228705752563807962227809675103689306"
    py:run "base1=base_"     ;; choose 2 or 5 states and adjust agent color and states accordingly
...
to update-turtles
  ...
  ask turtles [
    ...
    [
      ...
      ;; if agent is unhappy, he/she moves to try to find a better place
    if neighbor-left-state = 0 and neighbor-right-state = 0 [find-new-spot]
...
to find-new-spot
  ...
  [
    if graus > temperature + 0.3 [       ;; if arduino true and current temperature 0.5 degrees Celsius greater then initial temp, move faster
...
CA-rule
CA-rule
30 126 141 214 232 "2159062512564987644819455219116893945895958528152021228705752563807959237655911950549124"
4
...
The links in the social network are imported from a CSV file available at:
https://raw.githubusercontent.com/RubensZimbres/Repo-2022/main/SantaFe/NetLogo_final/Best_v3/social_80.csv You can create your own CSV file. Be sure that the number of individuals in the CSV file matches the N-turtles.
...
- EPOCHS: how many cycles will last the model run.
...
<experiments>
  ...
  <experiment name="experiment_centroid_links_mutated" repetitions="1" runMetricsEveryStep="true">
    ...
    <steppedValueSet variable="percentage-unhappy" first="10" step="30" last="40"/>
    <enumeratedValueSet variable="clear-before-generating">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="arduino-on">
