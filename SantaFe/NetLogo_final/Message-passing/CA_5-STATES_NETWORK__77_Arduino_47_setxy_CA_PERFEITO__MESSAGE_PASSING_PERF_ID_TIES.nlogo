...
to setup
  ...
  file-open "/home/theone/Downloads/NetLogo/relationships_matrix.graphml"                                    ;; add your path here

    ifelse CA-rule = 30 or CA-rule = 126 or CA-rule = 214 or CA-rule = 232 [
  ...
  ask turtles with [ breed != service-providers ] [
    set breed clients
     ...
     file-open "/home/theone/other_models/SantaFe/social_80.csv" ; open the file with the turtle data
  ;  let links csv:from-file "/home/theone/other_models/SantaFe/social.csv"
...
to make-node
  if breed = clients [
    ...
    set neighbor-right-state [state] of neighbor-right
      ...
      set color 42
      ask turtles [
        ask myself [set weight weight + 0.14 ] ]
      ]                                              ;; add weight to the edge to the current left neighbor

        create-link-with neighbor-right [
        set color 42
        ask myself [
        ask my-links [set weight weight + 0.14 ] ]
      ]
        ;; add weight to the edge to the current right neighbor
  ]
        []
...
to identify-turtles
  ...
  ifelse any? turtles with [distancexy mouse-xcor mouse-ycor < 1] [ ;;and member? nw:louvain-communities ([who] of turtles) [
...
to cellular_automata                                        ;; algorithm for the cellular automata
    py:set "rule_ca" CA-rule
    py:set "base_ca" CA-base
...
to message-passing
    ...
    py:run "Atil=np.array(adj)+np.eye(80)"
    py:run "D_mod = np.zeros_like(Atil)"
    py:run "np.fill_diagonal(D_mod, Atil.sum(axis=1).flatten())" ;;certo
    py:run "diagonal_degree_matrix = np.linalg.inv(sqrtm(D_mod))" ;certo
    py:run "A_hat=diagonal_degree_matrix @ Atil @ diagonal_degree_matrix"  ; certo
    (py:run "def norm(x):"
    ...
    (py:run "def pesos():"
            ...
            "    output=0.6+np.sum(A_hat,axis=1)*results[-79]"
            "    out2=len(np.where(output[0]>np.mean(output[0]))[0]/len(output))"
    "    return output,out2");"    return results[79].flatten()");np.array(results[79]).reshape(1,-1)")


  set info py:runresult "pesos()[0][0]"                                      ;; Message passing
  set spread py:runresult "pesos()[1]"                                       ;; Message passing

    (foreach info range 80
    [[ a b ] -> ask turtle b [ set size 0.2 + a ]
...
to resize-node
  ...
  ;ask turtles [ set size 0.8 + ( sqrt count link-neighbors ) / 7  ]            ;; size of node according to number of links with neighbors
                                                                               ;;"set label who" to see agents numbers
end

to update-turtles
  ;;set adjacency report-matrix
 ...
 ifelse CA-rule = 30 or CA-rule = 126 or CA-rule = 214 or CA-rule = 232 [
    ...
    ;if state = 0 [find-new-spot]                                     ;; if agent is unhappy, he/she moves to try to find a better place
...
to find-new-spot
  ...
  rt movement-steps + 2.5 * ( graus - temperature)
  fd movement-steps + 2.5 * ( graus - temperature)
...
CA-rule
CA-rule
30 126 214 232 "2159062512564987644819455219116893945895958528152021228705752563807959237655911950549124" "2159062512564987644819455219116893945895958528152021228705752563807962227809675103689306"
2
...
268
520
Rule 215906251256... requires base 5
12
0.0
1
...
Also, there is an algorithm for message passing in the social network, to analyze the flow and spread of information among nodes.
...
Then, activate the Python extension and configure the path, for instance: /home/anaconda3/bin/python

You will also need the CSV file to connect nodes, available at: https://raw.githubusercontent.com/RubensZimbres/Repo-2022/main/SantaFe/NetLogo_final/Best_v3/social_80.csv  Be sure to edit the CSV path in the code.
...
Regarding the message passing algorithm, it was also developed in Python from the adjacency matrix (A) collected from the NetLogo run. The calculation of information flow is given by the following algorithm:

- Calculate the Normalized Adjacency Matrix, by adding the Identity Matrix

![Normalized Adjacency Matrix](https://github.com/RubensZimbres/Repo-2022/blob/main/SantaFe/NetLogo_final/Message-passing/A.png?raw=true)

- Then calculate the Diagonal Degree Matrix

![Diagonal degree matrix] (https://github.com/RubensZimbres/Repo-2022/blob/main/SantaFe/NetLogo_final/Message-passing/D.png?raw=true)

Initially, only the agent wth more links have information equal to 1, then, this information spreads over the network, by sucessively multiplying this initial vector of information [1,0,0,0,0....] by the Diagonal Degree Matrix. Then, information flows over the network [0.6,0.22,0.27,0.1,0,0,0...].
...
The temperature is collected by a DHT11 sensor attached to Arduino and infuence the degree of movimentation of agents. In colder temperatures, agents move less, and in high temperatures, agents move more steps. This is similar to physical properties of the states of matter. 

It's also possible to enter the path where you want to save the model as GraphML.
...
The button IDs will show the labels of agents when mouse is over. The size of agents show how much information he/she carries.


## HOW TO USE IT

Click the SETUP button to set up the agents. You can choose equal numbers of red and green agents, but you can change it using PERCENTAGE-UNHAPPY. The agents are set up so no patch has more than one agent.  Click GO to start the simulation. If agents don't have enough neighbors in the radius of interaction, they move to a nearby patch. (Note that the topology is not wrapping).
...
When you stop the iterations, you can check the number of turtles unhappy, neutral and the number of turtles happy and the MEAN CLUSTERING COEFFICIENT of turtles. The strength of ties can be seen by the thickness of the connections among agents.
...
Try different setups. Maybe you will reach a state where the system may reach an equilibrium, stop moving and stop creating new links. In this case, incrase the temperature of the system in the Arduino DHT11 sensor e see what happens.
...
* Zimbres, R.A., Oliveira, P.P.B. (2009). Dynamics of Quality Perception in a Social Network: A Cellular Automaton Based Model in Aesthetics Services. Electronic Notes in Theoretical Computer Science, Elsevier, Volume 252, 1, Pages 157–180. https://www.sciencedirect.com/science/article/pii/S1571066109003740
...
<experiments>
  ...
  <experiment name="experiment_centroid_links_mutated" repetitions="1" runMetricsEveryStep="true">
    ...
    <metric>centroid-y</metric>
    <metric>count links</metric>
    <steppedValueSet variable="radius-of-interaction" first="1.5" step="4" last="9.5"/>
    <steppedValueSet variable="movement-steps" first="1" step="2" last="5"/>
    <enumeratedValueSet variable="load-graphml">
      <value value="false"/>
    </enumeratedValueSet>
    ...
    <steppedValueSet variable="percentage-unhappy" first="10" step="35" last="80"/>
    <enumeratedValueSet variable="clear-before-generating">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="arduino-on">
  ...
  <experiment name="experiment-CA-setxy-214" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean [ state ] of turtles with [ breed = clients ]</metric>
    <metric>mean [ state ] of turtles with [ breed = service-providers ]</metric>
    ...
    <steppedValueSet variable="radius-of-interaction" first="1.5" step="4" last="9.5"/>
    <steppedValueSet variable="movement-steps" first="1" step="4" last="9"/>
    <enumeratedValueSet variable="load-graphml">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="path-to-save">
      <value value="&quot;/home/theone/Downloads/NetLogo/relationships_matrix.graphml&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="layout">
      <value value="&quot;radial&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="percentage-unhappy">
      <value value="20"/>
      <value value="30"/>
      <value value="80"/>
    </enumeratedValueSet>
  ...
  <experiment name="experiment-CA-setxy-232" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean [ state ] of turtles with [ breed = clients ]</metric>
    <metric>mean [ state ] of turtles with [ breed = service-providers ]</metric>
  ...
  <experiment name="message-passing-232" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <steppedValueSet variable="frac-providers" first="0.05" step="0.1" last="0.15"/>
    <enumeratedValueSet variable="radius-of-interaction">
      <value value="1"/>
      <value value="4"/>
      <value value="9"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="movement-steps">
      <value value="1"/>
      <value value="4"/>
      <value value="9"/>
    </enumeratedValueSet>
