...
to message-passing
    ...
    py:set "adj" matrix:to-row-list adjacency
    py:run "import numpy as np"
  ...
  py:run "Atil=np.array(adj)+np.eye(80)"
    ...
    (py:run "def pesos():"
            ...
            "    return 0.5+np.sum(A_hat,axis=1)*np.sum(results,axis=1).flatten()");"    return results[79].flatten()");np.array(results[79]).reshape(1,-1)")


  set info py:runresult "pesos()";[0]"                                        ;; Message passing perfect
...
"" ""
PENS
"Degree" 1.0 1 -7858858 true "" "histogram [ count my-links ] of turtles"
...
CA-rule
CA-rule
30 126 214 232 "2159062512564987644819455219116893945895958528152021228705752563807959237655911950549124"
3
