...
globals [
  ...
  move-steps2                 ; equation sqrt (|delta-heat - delta-E|) / k
...
to setup
  ...
  file-open path-to-save                                    ;; add your path in the Input box in the interface
    ...
    ask turtles [
      ...
      ask n-of (frac-providers *  ( int ( N-turtles * perc-unhappy / 100  ) ) ) turtles [ ;; set breed service-providers
  ...
  crt int ( N-turtles * perc-unhappy / 100  )    ; create unhappy agents
    ...
    set size 0.8
      ...
      crt N-turtles - int ( perc-unhappy / 100 * N-turtles )  ; create agents with state bigger or equal to 2
    [setxy random-xcor random-ycor ; random coordinates
      ifelse CA-rule = 30 or CA-rule = 23 or CA-rule = 232 [
...
to make-node
  if breed = clients [
    set neighbor-left one-of turtles with [ distance myself <= radius-me ]    ;; choooses one of neighbors inside radius of interaction
       set neighbor-right one-of turtles with [ distance myself <= radius-me ]   ;; choooses one of neighbors inside radius of interaction
...
to cellular_automata                                        ;; algorithm for the cellular automata
  ...
  if arduino-on = True [
    ...
    set move-steps2 6 * sqrt ( abs ( delta-heat - delta-E ) ) ]   ;;; abs to avoid imaginary numbers // 0.16 constant (mass)
...
to find-new-spot
  ifelse arduino-on = true                                   ;; if Arduino on, move the move-steps2 calculated with thermodynamics
  [
   rt move-steps2
  fd move-steps2
  ...
  [
  rt move-steps                   ;; if arduino false, move regularly
  fd move-steps
