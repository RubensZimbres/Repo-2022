...
to setup
  ...
  file-open path-to-save                                    ;; add your path here
...
to layout-turtles
  ...
  if layout = "circle" [
    layout-circle turtles max-pxcor * 0.47
  ]
  display
end
...
to message-passing
    ...
    (py:run "def pesos():"
            ...
            "    results[0] = H"
            "    for i in range(1,80):"
            "        results[i] = np.array(np.array(A_hat) @ results[i-1].flatten()).reshape(-1,1)"  ;;;; bug here
