...
to message-passing
  ...
  py:run "Atil=np.array(adj)+np.eye(80)"
    ...
    "    return 0.65+np.sum(A_hat,axis=1)*results[-79]");"    return results[79].flatten()");np.array(results[79]).reshape(1,-1)")


  set info py:runresult "pesos()[0]";[0]"                                        ;; Message passing perfect
...
"" ""
PENS
"Time" 1.0 0 -7858858 true "" "carefully [plot ( sum py:runresult \"pesos()[0]\" )][]"
