...
to cellular_automata                                        ;; algorithm for the cellular automata
  ...
  if arduino-on = True [
    set delta-E ( state - py:runresult "cellular_automaton([a,b,c])[0]" )
    set delta-heat graus - temperature
    set movement-steps2 sqrt ( abs ( delta-heat - delta-E ) ) ]   ;;; abs to avoid imaginary numbers
...
to find-new-spot
  ...
  rt movement-steps2 + 2.5 * ( graus - temperature)
  fd movement-steps2 + 2.5 * ( graus - temperature)
...
This project was developed during the Santa Fe course Introduction to Agent-Based Modeling. The origin of this project was in my doctorate, when I used cellular automata (CA) to simulate human interactions that happened in the real world. I started with a market research with real people in two different times: one at time zero and the second at time zero plus 4 months (longitudinal market research). 
...
The temperature is collected by a DHT11 sensor attached to Arduino and influence the degree of movimentation of agents. In colder temperatures, agents move less, and in high temperatures, agents move more steps. This is similar to physical properties of the states of matter. 
...
The button IDs will show the labels of agents when the mouse is over. The size of agents shows how much information he/she carries.

The temperature sensor will generate N MOVEMENT-STEPS, according to the following equations:

![Equations](https://github.com/RubensZimbres/Repo-2022/blob/main/SantaFe/NetLogo_final/Message-passing/movement-stepss.png?raw=true)
...
Try to alter the amount of movement the agent does when he is unhappy or with few neighbors. You can also change the radius an agent will consider when interacting with others. Does a small radius isolate him/her in the social dynamics ?
