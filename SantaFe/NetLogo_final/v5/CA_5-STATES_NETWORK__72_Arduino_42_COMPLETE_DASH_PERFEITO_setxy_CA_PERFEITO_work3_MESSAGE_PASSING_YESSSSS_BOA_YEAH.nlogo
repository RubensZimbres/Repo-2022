...
to message-passing
    ...
    py:run "np.fill_diagonal(D_mod, Atil.sum(axis=1).flatten())"
    py:run "diagonal_degree_matrix = np.linalg.inv(sqrtm(D_mod))"
    py:run "A_hat=diagonal_degree_matrix @ Atil @ diagonal_degree_matrix"
    py:run "iters=80"
    (py:run "def norm(x):"
    ...
    (py:run "def pesos():"
            ...
            "        H = A_hat @ H"
            "    return 8*norm(np.sum(A_hat,axis=1))")
  set info py:runresult "pesos()"                                        ;; Message passing perfect
