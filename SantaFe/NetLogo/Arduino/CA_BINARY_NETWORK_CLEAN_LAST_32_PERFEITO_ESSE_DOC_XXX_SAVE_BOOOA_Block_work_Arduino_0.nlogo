...
to setup
  ...
  if load-matrix = True[
  file-open "relationships_matrix.txt"
...
to go
  ...
  ifelse count links > 100 [turn-led True
    set status True][set status False]
...
to save-matrix
  nw:set-context turtles links
  nw:save-graphml "relationships_matrix.graphml"
end

to load-matrix1
  generate [ -> nw:load-graphml "relationships_matrix.graphml"]
end
@#$#@#$#@
GRAPHICS-WINDOW
615
