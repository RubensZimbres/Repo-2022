...
to make-node [old-node]
  if breed = clients [
    set neighbor-left one-of other turtles in-radius radius-of-interaction with [ breed = clients ]   ;; choooses one of neighbors inside radius of interaction
       set neighbor-right one-of other turtles in-radius radius-of-interaction  with [ breed = service-providers ] ;; choooses one of neighbors inside radius of interaction
    if old-node != nobody
  ...
  set neighbor-left one-of other turtles in-radius radius-of-interaction with [ breed = service-providers ]   ;; choooses one of neighbors inside radius of interaction
       set neighbor-right one-of other turtles in-radius radius-of-interaction  with [ breed = service-providers ] ;; choooses one of neighbors inside radius of interaction
    if old-node != nobody
        [carefully[create-link-with neighbor-left [ set color 42
        set weight weight + 0.14                                               ;; add weight to the edge to the current left neighbor
...
to go
  ...
  path-L
      set centroid-x mean ( [xcor] of turtles  )
    set centroid-y mean ( [ycor] of turtles  )
...
"" ""
PENS
"clients" 1.0 0 -13345367 true "" "plot ( sum [ state ] of turtles with [ breed = clients ] ) /  (( 1 - frac-providers) * N-turtles)"
"service-providers" 1.0 0 -955883 true "" "plot ( sum [ state ] of turtles with [ breed = service-providers ] ) /  ( frac-providers * N-turtles )"
