...
to setup
  ...
  crt N-turtles - int ( percentage-unhappy / 100 * N-turtles ) ;create agents with state bigger or equal to 2
    ...
    ask n-of (frac-providers * ( N-turtles - int ( percentage-unhappy / 100 * N-turtles ) ) ) turtles [
    set breed service-providers
  ]
  ask turtles with [ breed != service-providers ] [
    set breed clients
  ...
  ;;layout-spring turtles links 0.2 5 2
...
to make-node [old-node]
  if breed = clients [
  set neighbor-left one-of other turtles in-radius radius-of-interaction with [ breed = clients ]   ;; choooses one of neighbors inside radius of interaction
       set neighbor-right one-of other turtles in-radius radius-of-interaction  with [ breed = clients ] ;; choooses one of neighbors inside radius of interaction
    if old-node != nobody
