...
globals [
  ...
  graus                           ; the temperature at each of the agents' interaction
  setup-yet?                      ; check if Arduino is propoerly set up
...
to setup
  ...
  file-open "/home/theone/Downloads/NetLogo/relationships_matrix.graphml"
    ask turtles [    if state = 0 [ set color red ]         ; colors of each one of the 5 states
...
to make-node [old-node]
    set neighbor-left one-of other turtles in-radius radius-of-interaction    ;; choooses one of neighbors inside radius of interaction
       set neighbor-right one-of other turtles in-radius radius-of-interaction ;; choooses one of neighbors inside radius of interaction
    if old-node != nobody
...
to cellular_automata                                        ;; algorithm for the cellular automata
    ...
    py:run "regra=2159062512564987644819455219116893945895958528152021228705752563807962227809675103689306" ;; rule of my thesis 2159062512564987644819455219116893945895958528152021228705752563807959237655911950549124
  ...
  let final-state py:runresult "cellular_automaton([a,b,c])[0]"            ;; assigns result of the cellular automaton to the agent state and color
...
to update-turtles
  ...
  ask turtles [
    ...
    if pcolor = white [forward 3]                                      ;; makes the agent move in case of disturbance
  ]
    [
...
to add-barrier                                                 ;;; adds disturbance to the system, by triple clicking in the lattice
  if mouse-down?
...
Install it. Then, if you are using Linux, run:

sudo chmod a+rw /dev/ttyACM0

Then, download the DHT library from Arduino and the .ino file from: 
...
In Arduino software, check for errors in the code, compile and upload to the board.

Open Tools / Monitor and check if the sensor is indeed reading the humidity and temperature.
...
Initially, each individual has its own state, and as an amorphous cellular automaton, he will interact with any of the other 2 individuals within the defined radius of interaction. The behavior of an agent is defined by the type of neighbors he/she has at time t. So, at time t+1 the states of all agents that interacted with two neighbors are updated according to the rule. 
...
- ARDUINO-ON: choose if Arduino is set to ON or OFF. If Arduino is ON and properly set up, it is supposed that humidity and temperature values show up. If not, check your installation.
- LINKS-TO-USE: if they are directed or indirected
- N-TURTLES: the N number of individuals (turtles)
...
The temperature is collected by a DHT11 sensor attached to Arduino and infuence the degree of movimentation of agents. In colder temperatures, agents move less, and in high temperatures, agents move more. This is similar to chemical principles of solid, liquid and gasses. At higher temperatures molecules are faster.


Iterative

The agents will randomly choose their left and right neighbors. In case there are not two neighbors within the radius defined in MINIMUM-SEPARATION, or if they are unhappy, they will move MOVEMENT-STEPs until they find two available neighbors. In this movement, relative radius is updated. As soon as they find two neighbors, they update their X,Y coordinates as the mean of neighbors and their state according to the colors of each one of the neighbors, following the transition table of the chosen CA rule.
...
If you triple click any area of the lattice, you will add a disturbance in the network (white area) and agents will avoid this area, influencing the spatial disposition of the network.
...
If you choose ARDUINO-ON, wait for temperature and humidity to show up. If they don't show up when you click SETUP, go to Arduino software and check if the sensor is really reading temperature and humidity. Only then and after 3 seconds, click Go.
...
When you click BIGGEST CLIQUES, you will find the individual with more cliques in the social network. A clique is a sub-set of a network in which the actors are more closely and intensely tied to one another than they are to other members of the network.

DETECT COMMUNITIES

You can also detect communitites. Netlogo interface will show each community with its members and correspondent colors. Detects community structure present in the network. It does this by maximizing modularity using the [Louvain method](https://en.wikipedia.org/wiki/Louvain_Modularity).

CLOSENESS

The Closeness button will show you the closeness of each agent. Closeness centrality indicates how close a node is to all other nodes in the network. It is calculated as the average of the shortest path length from the node to every other node in the network.
...
The three plots below will show the distribution of states, the degree distribution as an histogram and the mood evolution. Note that the mood output may be very similar to the oscillating behavior found by Brian Arthur.
...
Note what happens if the temperature of the environment goes up or down.
...
You will notice that the system will probably reach an equilibrium, stop moving and stop creating neu links. In this case, incrase the temperature of the system in the Arduino DHT11 sensor e see what happens.

Try also the opposite, when the social ties are forming, decrease temperature in the sensor and see if the system reaches an equilibrium.
...
Zimbres, R.A., Oliveira,P.P.B. (2009) Dynamics of Quality Perception in a Social Network: A Cellular Automaton Based Model in Aesthetics Services. Electronic Notes in Theoretical Computer Science 252 pp 157–180.

## HOW TO CITE

If you mention this model or the NetLogo software in a publication, we ask that you include the citations below.

For the cellular automata model itself:
