...
while m>0:
    ...
    edges=profs+clientes
    #print(dict(np.array([nodes,all]).reshape(-1,1)))
    nodes=np.arange(0,len(all))
    print(nodes)
    print(all)
    G = nx.Graph()
    G.add_nodes_from(nodes)
    G.add_edges_from(edges)
    groups0=nx.set_node_attributes(G, dict(np.transpose(np.array([nodes,all]))), name="color")
    groups=set(nx.get_node_attributes(G,'color').values())
    pos = nx.spectral_layout(G)
    list_degree=list(G.degree())
    nodes , degree = map(list, zip(*list_degree))
    #color_map = ['green' if node > len(clients) and 'white' if else 'red' for node in G] 
    figure(figsize=(20,16))
    #d = dict(G.degree())
    #nodes=G.nodes
    mapping = dict(np.transpose(np.array([nodes,all]))) #dict(zip(sorted(groups),count()))
    #print(mapping)
    colors = all
##    ec = nx.draw_networkx_edges(G, pos, alpha=0.2)
 #   nc = nx.draw_networkx_nodes(G, pos, nodelist=nodes, node_color=colors, 
 #                           cmap=plt.cm.jet,node_size=[(v * 9)+1 for v in degree])
    nx.draw(G, pos, with_labels=True, font_weight='light',cmap=plt.cm.jet,linewidths=2,width=0.3,nodelist=nodes, node_color=colors,node_size=[(v * 9)+1 for v in degree])


    #plt.colorbar(nx.draw_networkx_nodes(G, pos, nodelist=nodes, node_color=colors, 
    #                        cmap=plt.cm.jet,node_size=[(v * 9)+1 for v in degree]),fraction=0.0046, pad=0.004)
    #plt.axis('off')
    ##nx.draw(G, pos, with_labels=True, font_weight='light',linewidths=2,width=0.3,node_color=color_map,node_size=[(v * 9)+1 for v in degree])
